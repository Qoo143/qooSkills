---
name: flightphp
description: FlightPHP 框架最佳實踐指南。當撰寫 FlightPHP 路由、Controller、中間件、資料庫操作，或建立 API v4 端點時觸發。
---

# FlightPHP 最佳實踐開發指南

> **目標讀者**：AI 編碼助手。此 Skill 提供基於 flightphp/core 原始碼的精確 API 用法與最佳實踐，避免生成不存在的 API 或錯誤的呼叫方式。
...
> 開發工作流（端點分析、實作流程）已移至 `api-v4` skill。
